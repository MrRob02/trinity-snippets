...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('trinity.wrapWithSignalBuilder', async () => {
        ...
        const snippet = new vscode.SnippetString(`SignalBuilder<\${2:StateType}>(\n` +
        ...
        const line = editor.document.lineAt(range.start.line);
        const indent = line.text.match(/^\s*/)?.[0] || '';
        const snippet = new vscode.SnippetString(`NodeProvider(\n` +
        ...
        const line = editor.document.lineAt(range.start.line);
        const indent = line.text.match(/^\s*/)?.[0] || '';
        const snippet = new vscode.SnippetString(`SignalListener<\${1:StateType}>(\n` +
    ...
    context.subscriptions.push(vscode.languages.registerCodeActionsProvider('dart', new TrinityCodeActionProvider(), {
        providedCodeActionKinds: [vscode.CodeActionKind.Refactor]
    }));
}
class TrinityCodeActionProvider {
    provideCodeActions(document, range) {
        const actions = [];
        const wrapSignal = new vscode.CodeAction('Wrap with SignalBuilder', vscode.CodeActionKind.Refactor);
        wrapSignal.command = {
            command: 'trinity.wrapWithSignalBuilder',
            title: 'Wrap with SignalBuilder'
        };
        const wrapNode = new vscode.CodeAction('Wrap with NodeProvider', vscode.CodeActionKind.Refactor);
        wrapNode.command = {
            command: 'trinity.wrapWithNodeProvider',
            title: 'Wrap with NodeProvider'
        };
        const wrapListener = new vscode.CodeAction('Wrap with SignalListener', vscode.CodeActionKind.Refactor);
...
function deactivate() { }
function getWidgetRange(document, position) {
    ...
    if (!widgetMatch || widgetMatch.index === undefined)
        return null;
    ...
    if (depth !== 0)
        return null;
    return new vscode.Range(document.positionAt(nameStart), document.positionAt(parenEnd));
}
